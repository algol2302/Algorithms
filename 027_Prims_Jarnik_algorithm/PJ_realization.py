...
class PrimsJarnikAlgorithm:
    ...
    def find_spanning_tree(self, start_vertex: Vertex):
        ...
        while self.unvisited_list:
            ...
            # it may happend that we would visited the vertex
            # we have already considered
            if min_edge.target_vertex in self.unvisited_list:
                self.mst.append(min_edge)
                print(
                    f"Edge added to spanning tree: "
                    f"{min_edge.start_vertex.name} - "
                    f"{min_edge.target_vertex.name}"
                )
