...
class DijkstraAlgorithm:
    ...
    def get_min_vertex(self):
        ...
        # linear search in O(N) linear running time
    ...
    def calculate(self):
        ...
        for vertex in range(self.v):
            ...
            # it has again O(V) running time -
            # so the overall running time is O(V^2)
            for other_vertex in range(self.v):
                ...
                if self.adjacency_matrix[actual_vertex][other_vertex] > 0:
                    # is there a shorter path to the other_vertex
                    # from the actual_vertex
                    if (
